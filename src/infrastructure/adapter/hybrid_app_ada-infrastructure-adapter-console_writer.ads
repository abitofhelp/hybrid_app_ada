...
--    Implements the signature required by Hybrid_App_Ada.Application.Port.Out.Writer.
--    Uses Functional.Try to convert exception-prone I/O to Result types.
--
--  Architecture Notes:
--    - ADAPTER that implements the Writer output port
--    - Conforms to interface defined by Hybrid_App_Ada.Application.Port.Out.Writer
--    - Uses Functional.Try.Try_To_Result_With_Param for exception handling
--    - Depends on Ada.Text_IO (standard library IO)
--    - This layer depends on Hybrid_App_Ada.Application + Hybrid_App_Ada.Domain + Functional
--
--  Dependency Flow:
--    Console_Writer -> Hybrid_App_Ada.Application.Port.Out.Writer (implements interface)
--    Console_Writer -> Functional.Try (exception boundary conversion)
--    Console_Writer -> Hybrid_App_Ada.Domain.Result (for error handling)
--    Console_Writer -> Ada.Text_IO (external IO library)
--
--  See Also:
--    Hybrid_App_Ada.Application.Port.Out.Writer - Port interface this implements
--    Functional.Try - Exception-to-Result bridge used internally
--  =========================================================================

with Hybrid_App_Ada.Application.Port.Outbound.Writer;

package Hybrid_App_Ada.Infrastructure.Adapter.Console_Writer is
